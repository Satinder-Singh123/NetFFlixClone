...
import { getAllfavorites, getSimilarTVorMovies, getTVorMovieDetailsByID } from "@/util";
import ReactPlayer from "react-player";
import MediaItem from "../media-item";
import { useRouter } from "next/navigation";
import { useSession } from "next-auth/react";

export default function DetailPopup({ show, setShow }) {
  const {
    mediaDetails,
    setMediaDetails,
    similarMedias,
  ...
  useEffect(() => {
    if (currentMediaInfoIDAndType !== null) {
      async function getMediaDetails() {
        ...
        const allfavourites = await getAllfavorites(
          session?.user?.uid,
          loggedInAccount?._id
        );
        console.log(extractMediaDetails, "ectractmedia");
        ...
        setSimilarMedias(
          extractSimilarMovies.map((item) => ({
            ...
            addedToFavorites:
            allfavourites && allfavourites.length
              ? allfavourites
                  .map((favo) => favo.movieID)
                  .indexOf(item.id) > -1
              : false,
          }))
        );
      ...
      >
        <div>
          ...
          <div className="rounded-b-md bg-[#181818] p-8">
            <div className="space-x-2 pc-4 flex gap-4">
            ...
            <h2 className="mt-8 mb-6 cursor-pointer text-sm font-semibold text-[#e5e5e5] transition-colors duration-200 hover:text-white md:text-2xl">
              More Like This
            </h2>
